# Distribución — Política Sencilla RD

Sitio: https://kevthetech143.github.io/leyes-rd/

## Aprobación
- 2026-06-08 — Kelvin auto-aprobó publicación/distribución automática.
- Límites que se mantienen aun con aprobación:
  1. NUNCA mensajes masivos ni mensajes en frío a desconocidos (spam → baneo de cuentas, viola términos de plataforma, leyes anti-spam).
  2. Solo cuentas propias de Kelvin y canales/grupos públicos donde se permite compartir.
  3. Si un canal necesita un login que no tenemos conectado → registrar el hueco aquí y avisar a Kelvin. Nunca fingir que se publicó.

## Estado de canales (qué tenemos conectado de verdad)
- Telegram — conectado (bots + MCP). Alcance real depende de tener un canal/grupo con suscriptores.
- X / Twitter — NO conectado. Falta login.
- Instagram — NO conectado. Falta login.
- Facebook — NO conectado. Falta login.
- WhatsApp — compartir manual sí (botón en el sitio); difusión automática NO conectada.

ACCIÓN PENDIENTE DE KELVIN: decir desde qué cuenta(s) publicamos, o iniciar sesión en el navegador /web para X/IG/FB.

## Contador de tráfico
- GoatCounter (gratis, sin cookies). Falta crear la cuenta (~2 min). Pasos:
  1. Entrar a https://www.goatcounter.com/signup
  2. Elegir un código de sitio (ej. politicasencillard).
  3. Pegar el script que dan en el <head> de docs/index.html.
- Sin la cuenta NO se muestra ningún número. Cero números inventados.

## Posts listos para publicar

### Corto (X / estados / grupos)
🇩🇴 ¿Quieres entender la política dominicana sin complicarte?

Política Sencilla RD: las leyes, tu provincia, quién manda y cómo se mueve el dinero público. Explicado fácil, gratis, sin registro.

👉 https://kevthetech143.github.io/leyes-rd/

### WhatsApp / familiar
Mira esto: una página gratis que explica la política de RD bien fácil.
Puedes ver las leyes, quién está a cargo en tu provincia, el Senado y cómo se gasta el dinero público.
Sin cuenta, sin login. Pásalo. 🇩🇴
https://kevthetech143.github.io/leyes-rd/

### Largo (Facebook / descripción)
Política Sencilla RD nació para una idea simple: cualquier dominicano debería poder entender su gobierno sin ser experto.

En la página encuentras:
• Las leyes agrupadas por tema, explicadas en lenguaje claro.
• Tu provincia: senador, diputados, gobernador y alcaldes, con qué hace cada cargo.
• El Senado: sesiones y cómo se vota.
• El dinero público: cómo se mueve y dónde se puede romper.

Gratis. Sin registro. Para compartir.
https://kevthetech143.github.io/leyes-rd/

## Posts del caso SENASA (con imagen lista: docs/share-senasa.png — 1080×1080)

### WhatsApp / corto
🔎 Caso real: el desfalco del SENASA — más de RD$15,000 millones del dinero de la salud.
¿Dónde se rompió la cadena? Te lo explico fácil, paso a paso.
https://kevthetech143.github.io/leyes-rd/ (pestaña "Dinero")

### Facebook / largo
El SENASA es el seguro de salud del Estado, para 7.5 millones de personas. Se perdieron más de RD$15,000 millones.
En Política Sencilla RD lo explicamos paso a paso: dónde se rompió la cadena del dinero y cómo se evita. Sin palabras difíciles, gratis.
👉 https://kevthetech143.github.io/leyes-rd/
(Adjuntar imagen: share-senasa.png)

## Registro de publicaciones (fecha — canal — enlace)
- (vacío — ninguna publicación real todavía)
